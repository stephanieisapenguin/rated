...
|-------------|------------------------------------------|----------------------------|
| Local dev   | `http://localhost:8000`                  | none (localhost, open)     |
| Production  | `https://rated-api.onrender.com`         | pre-signed URL (`VITE_API_AUTH`) |

Auth is a pre-signed query string appended to every request — no Bearer header
needed. The signature is stored as `VITE_API_AUTH` (never committed).

### Setting up `.env.local` from GitHub secrets

Canonical values live in the repo's GitHub secrets
(`Settings → Secrets and variables → Actions`).
Copy them locally:

```bash
cp .env.example .env.local
# Then fill in the two secret values:
#   VITE_API_BASE_URL  →  secret: VITE_API_BASE_URL
#   VITE_API_AUTH      →  secret: VITE_API_AUTH
# VITE_GOOGLE_CLIENT_ID is already set in .env.example (it's public).
```

Or if you have the `gh` CLI:

```bash
gh secret list -R stephanieisapenguin/rated
# Copy VITE_API_BASE_URL and VITE_API_AUTH values into .env.local
```

The frontend reads these via `import.meta.env.VITE_*` (Vite injects at build
time). When the backend is unreachable the UI falls back to mock data.
